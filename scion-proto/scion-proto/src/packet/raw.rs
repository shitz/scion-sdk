...
use super::{
    ...
    InadequateBufferSize, ScionHeaders, ScionPacket, ScionPacketScmp, ScionPacketUdp,
};
use crate::{
    address::ScionAddr,
    path::DataPlanePath,
...
impl From<ScionPacketUdp> for ScionPacketRaw {
    fn from(p: ScionPacketUdp) -> Self {
        Self {
            headers: p.headers,
            payload: p.datagram.encode_to_bytes_vec().concat().into(),
        }
    }
}

impl From<ScionPacketScmp> for ScionPacketRaw {
    fn from(p: ScionPacketScmp) -> Self {
        ScionPacketRaw {
            headers: p.headers,
            payload: p.message.encode_to_bytes_vec().concat().into(),
        }
    }
}
